...
from .saas_payments import create_checkout_session, activate_checkout, handle_stripe_webhook
...
def activate_pilot_checkout(checkout_id: str, customer_email: Optional[EmailStr] = None) -> Dict[str, Any]:
    """Pilot helper for non-production testing when Stripe secrets are not configured. Disable before production launch."""
    ...
    result = activate_checkout(checkout_id, customer_email=str(customer_email) if customer_email else None)
...
async def stripe_webhook(request: Request) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Stripe webhook rejected: {exc}") from exc
